...
import { XIcon, BuildingIcon, UploadIcon } from './icons';
...
export const AddPropertyModal: React.FC<AddPropertyModalProps> = ({ onClose, onAddProperty }) => {
  const [formData, setFormData] = useState<Omit<Property, 'id'>>({
    title: '', price: 0, type: 'Apartment', imageUrl: '', bedrooms: 0, bathrooms: 0, area: 0,
    description: '', amenities: [], gallery: [],
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl w-full max-w-3xl transform transition-all" onClick={e => e.stopPropagation()}>
        <div className="flex justify-between items-center p-5 border-b border-gray-200 dark:border-gray-700">
          <div className="flex items-center">
            <BuildingIcon className="w-6 h-6 text-blue-500 mr-3" />
            <h2 className="text-xl font-semibold text-gray-800 dark:text-gray-100">Adicionar Novo Imóvel</h2>
          </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex justify-end items-center p-5 bg-gray-50 dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 rounded-b-lg space-x-4">
            ...
            <button type="submit" className="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">Salvar Imóvel</button>
          </div>
        </form>
      </div>
    </div>
